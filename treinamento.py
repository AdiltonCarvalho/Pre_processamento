...
def treina_modelo(df_sem_nulos):
    ...
    dataset= df_sem_nulos.sample(40000)

    # Remove as colunas 'data_ref' e 'index' do DataFrame 'dataset'
    dataset.drop(['data_ref', 'index'], axis=1, inplace=True)

    experiment = setup(data=dataset, target='mau', experiment_name='credit_1', normalize=True, normalize_method='zscore', 
    ...
    nome_modelo = f'GBC Modelo {data_atual}'
